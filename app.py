...
def generate_workforce_data():
    """Generate synthetic Saudi workforce dataset — no external CSV needed."""
    np.random.seed(42)
    random.seed(42)
    ...
    NAT_KEYS      = ["Saudi","Indian","Pakistani","Bangladeshi","Egyptian","Filipino","Yemeni","Syrian","Jordanian","Indonesian","Sudanese","Other"]
    NAT_PROBS     = [0.35, 0.18, 0.10, 0.08, 0.07, 0.06, 0.05, 0.04, 0.03, 0.02, 0.01, 0.01]
    EDU_LEVELS    = ["High School","Diploma","Bachelor","Master","PhD"]
    EDU_W_SAUDI   = [0.15, 0.18, 0.45, 0.18, 0.04]
    EDU_W_OTHER   = [0.30, 0.22, 0.38, 0.08, 0.02]
    ...
    NAME_POOL = ["Al Rajhi","SABIC","Saudi Aramco","STC","Almarai","Jarir","Al Babtain",
                 "Riyad","Samba","NCB","Al Jazira","Olayan","Bin Laden","Al Futtaim",
                 "Maaden","ACWA","Saudi Cables","Mobily","Zain","Savola","Tawuniya"]
    SUFFIXES  = ["Co.","Ltd.","Corp.","Group","Holdings","Solutions","Services","International"]
    ...
    for i in range(120):
        ...
        companies.append({
            ...
            "company_name": f"{random.choice(NAME_POOL)} {random.choice(SUFFIXES)}",
            "sector": sec,
...
def build_company_features():
    ...
    for cid, g in df.groupby("company_id"):
        ...
        trend           = recent_saud_pct - older_saudi_pct
        ...
        records.append({
            ...
            "edu_bachelor_pct":     round(edu_bachelor_pct, 2),
...
def train_models(co):
    ...
    for name, model in models.items():
        ...
        results[name] = {
            "model":  model, "y_test": y_test, "y_pred": y_pred, "y_prob": y_prob,
    ...
    importances    = pd.DataFrame({
        "Feature":    FEATURES,
        "Importance": results[best_tree_name]["model"].feature_importances_,
    ...
    co["risk_tier"] = pd.cut(
        co["predicted_breach_prob"], bins=[0, 0.35, 0.65, 1.0],
...
# ── Section 1: ROC + PR Curves + Confusion Matrix ────────────────────────────
...
with mc1:
    ...
    for name, r in results.items():
        ...
        fig_roc.add_trace(go.Scatter(x=fpr, y=tpr, name=f"{name} (AUC={r['auc']:.2f})",
...
with mc2:
    ...
    for name, r in results.items():
        ...
        fig_pr.add_trace(go.Scatter(x=rec, y=prec, name=f"{name} (AP={r['ap']:.2f})",
            mode="lines", line=dict(color=colors_roc[name], width=2.5)))
...
with mc3:
    ...
    fig_cm = px.imshow(cm, text_auto=True,
        ...
        title=f"Confusion Matrix — {model_choice}", aspect="equal")
...
with fi1:
    fig_fi = px.bar(
        ...
        color="Importance", color_continuous_scale=["#0a1630","#1565c0","#38d9f5"],
...
with fi2:
    ...
    fig_thresh.add_trace(go.Scatter(x=thresh_full, y=prec_sw, name="Precision", line=dict(color="#38d9f5", width=2)))
    fig_thresh.add_trace(go.Scatter(x=thresh_full, y=rec_sw,  name="Recall",    line=dict(color="#9333ea", width=2)))
    fig_thresh.add_trace(go.Scatter(x=thresh_full, y=f1_arr,  name="F1",        line=dict(color="#f59e0b", width=2.5)))
    ...
    fig_thresh.update_layout(
        title="Precision / Recall / F1 vs Threshold", height=360,
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font_color="#5a85aa", title_font_color="#e0f0ff",
        margin=dict(l=10,r=10,t=40,b=10),
...
with dp1:
    ...
    fig_hist.add_trace(go.Histogram(
        ...
        nbinsx=20, name="Actual: Compliant", marker_color="#1565c0", opacity=0.75))
    fig_hist.add_trace(go.Histogram(
        x=fco_sect[fco_sect["will_breach"]==1]["predicted_breach_prob"],
        nbinsx=20, name="Actual: Breach",    marker_color="#ef4444", opacity=0.75))
    ...
    fig_hist.update_layout(
        barmode="overlay", title="Distribution of Predicted Breach Probabilities", height=330,
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font_color="#5a85aa", title_font_color="#e0f0ff",
        margin=dict(l=10,r=10,t=40,b=10),
...
fig_s2 = px.scatter(
    ...
    color="risk_tier", size="headcount", hover_name="company_name",
    hover_data={"sector":True,"region":True,"saudization_pct":":.1f",
                "gap_to_target":":.1f","predicted_breach_prob":":.3f","headcount":True},
...
# ── Section 5: Company Prediction Cards ──────────────────────────────────────
    ...
    <div class="pred-card {card_cls}">
      <div style="flex:1">
        <div style="color:#c8dff5;font-weight:600;font-size:0.88rem;">
          {row['company_name']}
          <span style="color:#2a5080;font-weight:400;font-size:0.78rem;">· {row['sector']} · {row['region']}</span>
        </div>
      ...
      <div style="text-align:right;margin-left:1.5rem;min-width:120px;">
        ...
        <span class="risk-badge-{card_cls}" style="margin-top:0.3rem;display:inline-block;">{tier}</span>
      </div>
...
📊 <b>Top Predictive Signal:</b> <b>{top_feat}</b> ranks as the most discriminative feature.
This means it varies most systematically between companies that breach and those that
comply — see the feature importance chart for the full ranked list.<br><br>
⚠️ <b>Risk Distribution:</b> Of {len(co_pred)} companies, <b>{high_count}</b> are High Risk
and <b>{med_count}</b> are Medium Risk at threshold <b>{threshold:.0%}</b>.<br><br>
